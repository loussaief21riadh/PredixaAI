...
from app.ai.v6b_clean.ranking_model import (
...
class V6BCleanWalkForwardBacktester:
    """
    Predixa AI V6B-CLEAN - Global Ranking Walk-Forward.
    ...
    VERSION = "V6B-CLEAN-GLOBAL-RANKING-PURGED-T2"
    ...
    def run(
        ...
        for test_position, target_index in enumerate(
            ...
            logger.info(
                "V6B-CLEAN walk-forward %s/%s | target=%s",
                test_position,
                test_draws,
        ...
        if evaluated == 0:
            raise ValueError(
                "No V6B-CLEAN draws were evaluated."
            )
